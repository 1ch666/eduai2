...
capabilities新增npcAi/caseGenerationAi，反映金鑰配置與明確false開關。這不等於測試供應商成功；頁面會區分舊後端缺欄位、未配置和已配置。查線上時舊端點courtAi:false、textAi:true，不能據此認定NPC已成功連線。
8項入口／模板測試、tsc、前端檢查通過。本批只改網頁入口與能力回報，Unity二進位未變；模板與play/index同步修改。未使用正式帳號或實際消耗AI額度，完整雲端案件→NPC模型回覆仍待正式部署後實測。
重要：須由使用者部署最新Worker，才有最新/court/頁面與能力欄位；GitHub Pages更新不能替代Worker部署。不得再把固定遊戲或模型選擇式回覆說成已完成完整AI生成對話。
