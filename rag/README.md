...
目前 `/api/ai/ask` 對明確詞義題直接回傳命中條目的完整原文與來源，不讓 LLM 改寫後冒稱教育部原文。雲端 NPC 也使用同一檢索器，回覆標示「教育部辭典」；超過 12,000 字的詞條不送入 Unity 對話，而保留原 NPC 問答流程，不截斷原文。NPC 畫面顯示最近五則，完整紀錄仍在伺服器。不把字典當法律條文、法官判決或證人見聞。RAG 的向量重排及與生成回答整合仍待後續驗證，不宣稱此資料包就能修好自由 NPC 對話。
